...
def main():
    ...
    for i, test_text in enumerate(test_texts, 1):
        ...
        try:
            ...
            print(
                f"Prediction: {'Positive' if prediction == 1 else 'Negative'} (confidence: {confidence:.4f})"
            )
                ...
                try:
                    ...
                    for word in words:
                        ...
                        if len(word_attrs) > 0:
                            avg_attr = sum(word_attrs) / len(word_attrs)
                            bar_length = (
                                int((avg_attr / max_attr) * bar_width) if max_attr > 0 else 0
                            )
                            bar = "█" * bar_length
                            print(f"{word:>15} | {bar:<40} {avg_attr:.4f}")
                        char_idx += word_len + 1  # +1 for space
                    ...
                    if word_scores:
                        top_word, top_score = max(word_scores, key=lambda x: x[1])
                        print(
                            f"💡 Most influential word: '{top_word}' (avg score: {top_score:.4f})"
                        )
